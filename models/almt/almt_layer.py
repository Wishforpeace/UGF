...
class HhyperLearningLayer(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):
        ...
        self.attend = nn.Softmax(dim = -1)
        self.to_q = nn.Linear(dim, inner_dim, bias=False)
        self.to_k_tt = nn.Linear(dim, inner_dim, bias=False)
        self.to_k_ta = nn.Linear(dim, inner_dim, bias=False)
        self.to_k_tv = nn.Linear(dim, inner_dim, bias=False)
        self.to_v_tt = nn.Linear(dim, inner_dim, bias=False)
    ...
    def forward(self, h_t, h_a, h_v, h_hyper):
        ...
        k_tt = self.to_k_tt(h_t)
        v_ta = self.to_v_ta(h_a)
        v_tv = self.to_v_tv(h_v)
        # v_tt = self.to_v_tv(h_t)


        q, k_tt,k_ta, k_tv, v_ta, v_tv = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=h), (q, k_tt,k_ta, k_tv, v_ta, v_tv))
        ...
        dots_tt = einsum('b h i d, b h j d -> b h i j', q, k_tt) * self.scale
        attn_tt = self.attend(dots_tt)

        h_hyper_shift = self.to_out(out_ta + out_tv)
        h_hyper += h_hyper_shift
        return h_hyper,attn_tt,attn_ta,attn_tv
...
class HhyperLearningEncoder(nn.Module):
    ...
    def forward(self, h_t_list, h_a, h_v, h_hyper):
        for i, attn in enumerate(self.layers):
            h_hyper,attn_tt,attn_ta,attn_tv = attn[0](h_t_list[i], h_a, h_v, h_hyper)
        return h_hyper,attn_tt,attn_ta,attn_tv
