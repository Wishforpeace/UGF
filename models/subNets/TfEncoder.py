...
class PositionEncodingTraining(nn.Module):
    ...
    def forward(self, embeddings):
        embeddings_min = embeddings.min(dim=1, keepdim=True)[0]
        embeddings_max = embeddings.max(dim=1, keepdim=True)[0]
        embeddings = (embeddings - embeddings_min) / (embeddings_max - embeddings_min + 1e-6)

        batch_size = embeddings.shape[0]
       
        # 执行 self.proj(embeddings)
